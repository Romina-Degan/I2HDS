...
cancerReg <- cancerReg %>% arrange(year) %>% mutate(MA=rollmean(cancerReg$measure ,k=14, fill=NA, align="right"))
print(cancerReg)
ggplot(cancerReg, aes(x = year, y = measure, col = area_name, position_jitter(width = 0.5, height = 0.5))) +
    geom_point(size = 1.0) +

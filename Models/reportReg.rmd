...
```{r}
wide_data <- cancerReg %>% select(-MA, -area_code) %>%
  group_by(area_name, year) %>%  # Group by area_name and year

  pivot_wider(
    names_from = area_name,   # Columns will be created from the 'year' variable
    values_from = measure # Values in the new columns will come from the 'measure' variable
  )
...
cancerReg <- cancerReg %>%
    arrange(year) %>%
    mutate(MA = rollmean(cancerReg$measure, k = 1, fill = NA, align = "right"))
